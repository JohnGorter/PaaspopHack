...
window.onload = function () {
    setInterval(function () {
        generateApollo(camdata02.level);
    }, 250);
    setInterval(function () {
        generatePhoenix(camdata05.level);
    }, 250);
};
...
function generateApollo(status) {
    ...
    if (status == 1) {
        for (var i = 0; i < 3; i++) {
            top_position = getRandomInt(0, 120);
            left_position = getRandomInt(-10, 75);
    ...
    if (status == 3) {
        ...
        for (var i = 0; i < 3; i++) {
            top_position = getRandomInt(0, 100);
            left_position = getRandomInt(-10, 75);
